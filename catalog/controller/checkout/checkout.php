...
class ControllerCheckoutCheckout extends Controller { 
	...
	public function guest_validate() {
		...
		if (!$this->config->get('config_checkout_guest') || $this->config->get('config_customer_price') || $this->cart->hasDownload()) {
			//$json['redirect'] = $this->url->link('checkout/checkout', '', 'SSL');
			$json['error']['warning'] = $this->language->get('error_need_register');
		}
	...
	public function shipping_method($render = true, &$data = array()) 
	{
		...
		$new_address = false;
		if(isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new' ) $new_address = true;

		if($new_address && isset($this->request->post['shipping_country_id'])){
			...
			// de antes, pero y si cambia? por eso el shipping de arriba
		...
		if(isset($country_id)){
			if($new_address){
				...
				$this->session->data['guest']['shipping'] = array_merge($shipping_address, 
																		...
																		array('country_id' => $country_id, 
																			  'zone_id' => $zone_id,
																			  'city' => isset($this->request->post['city']) ,
